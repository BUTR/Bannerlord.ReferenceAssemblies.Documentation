...
The landing page is finished here as well: the generator leaves a {{DELTA}}
token in index.md because the counts only exist after metadata.

Usage:  apply-api-delta.py <docs-dir>
Reads   <docs-dir>/api-delta.json, written by generate-docfx-config.py.
"""
...
def main():
    ...
    if "{{DELTA}}" not in body:
        sys.exit("error: %s has no {{DELTA}} token; was it generated by generate-docfx-config.py?" % index)
    if any(stats for _, stats in results):
        text = delta_table(results, titles)
    elif plan["derived"]:
        text = ("Nothing for this version: the Server and ModdingKit builds expose the same public "
                "surface as the client build.")
    else:
        text = "This version has no Server or ModdingKit package."
    with open(index, "w", encoding="utf-8", newline="\n") as fh:
        fh.write(body.replace("{{DELTA}}", text))
